...
// Keep these values in the API bundle. Vercel deploys this function independently
// and does not include arbitrary files from src/ beside it.
const MANGLIK_TITLE = 'Manglik Dosha Calculator — Free Mangal Dosha Check Online | Veadicastro';
const MANGLIK_DESCRIPTION = 'Check Manglik Dosha using birth date, time and place. See Mars placement from Lagna, Moon and Venus with Swiss Ephemeris, Lahiri ayanamsa and clear explanations.';
const MANGLIK_URL = 'https://veadicastro.in/manglik-dosha-calculator';
const MANGLIK_DATE_PUBLISHED = '2026-09-06';
const MANGLIK_DATE_MODIFIED = '2026-09-06';
const manglikFaqs: [string, string][] = [
  ['What is Manglik Dosha or Mangal Dosha?', 'Manglik Dosha, also called Mangal Dosha or Kuja Dosha, is a traditional Vedic astrology classification based on the house occupied by Mars.'],
  ['How is Manglik Dosha calculated?', 'The calculator uses birth date, local birth time and birthplace to calculate a sidereal chart, then counts Mars from Lagna, Moon and Venus.'],
  ['Can Manglik Dosha be cancelled?', 'Some traditions use cancellation or mitigating rules. This calculator shows selected factors for review and does not label a placement automatically cancelled.'],
  ['Is Manglik Dosha real?', 'Manglik Dosha is a traditional Vedic astrology concept. It is not scientifically established as a method for predicting marriage outcomes.'],
  ['How do I know if I am Manglik?', 'The primary result checks whether Mars occupies houses 1, 2, 4, 7, 8 or 12 from Lagna. Moon and Venus checks are shown separately.'],
  ['Can I check Manglik Dosha with only a birth date?', 'A Lagna-based check requires birth time and birthplace as well.'],
  ['Why can two calculators show different results?', 'They may use different birth details, timezones, ayanamsas, reference points or house rules.'],
  ['Is Moon-based Manglik the same as Lagna-based Manglik?', 'No. Each counts Mars from a different sign, so the results can differ.'],
  ['Does this calculator check cancellation factors?', 'It flags selected Mars dignity and Jupiter relationships as factors for review.'],
  ['Can a Manglik marry a non-Manglik?', 'A label should not decide a marriage. Consent, shared values and communication matter.'],
  ['Does Manglik Dosha end after age 28?', 'The natal Mars position does not change with age.'],
  ['Does it work for births outside India?', 'Yes, when the selected location supplies a supported timezone for the birth date.'],
  ['Are full and partial Manglik labels universal?', 'No. Their meaning varies between traditions and services.'],
  ['Why does the calculator need a birthplace?', 'Latitude, longitude and local timezone are needed to calculate the ascendant accurately.'],
];
const manglikSections = [
  { title: 'What is Manglik Dosha?', text: 'Manglik Dosha, also called Mangal Dosha, Kuja Dosha or Chevvai Dosham, is a traditional Vedic astrology classification based on Mars placement in a birth chart. Many practitioners check whether Mars is placed in the first, second, fourth, seventh, eighth or twelfth house from the ascendant. This calculator states that rule directly and shows the house found, rather than returning an unexplained label. A Manglik classification is a chart convention, not a measure of character, worth, ability to love or suitability for marriage. It cannot establish that a relationship will succeed or fail. The useful first step is to check the exact astronomical position, understand the stated rule, and then keep any traditional interpretation in its proper context.', link: '/blog/manglik-dosha-myths-vs-reality', anchor: 'our Manglik Dosha guide', after: 'explains why a single placement should never become a verdict on a relationship.' },
  { title: 'How this Mangal Dosha calculator works', text: 'Enter your date of birth, the local time recorded at birth, and the place of birth. Select the matching place from the search results so the calculator can use its latitude, longitude and timezone. The selected local time is resolved for the actual birth date, including daylight-saving changes where relevant. The chart then uses Swiss Ephemeris positions with Lahiri ayanamsa and whole-sign houses. Mars is counted from the ascendant as the primary result and from Moon and Venus as separate supplementary checks. Showing all three references avoids hiding different traditions behind one vague score. The calculator result remains free and does not require a signup.', link: '/free-kundli-generator', anchor: 'the free Kundli generator', after: 'provides a complete view of planets, houses and Nakshatras.' },
  { title: 'Why birth time, latitude and longitude matter', text: 'A birthplace is not only a city name. Its coordinates influence the ascendant, and its timezone determines how local birth time becomes Universal Time for the astronomical calculation. Two people born on the same date can have different ascendants because the Earth rotates throughout the day. Near an ascendant boundary, a small time difference can change the whole-sign house occupied by Mars. This calculator therefore does not silently use a default city. If your birth time is approximate, compare plausible recorded times and treat a changing result as uncertainty rather than forcing a certain answer. Very old records may need extra care because historical local-time practices can differ.', link: '/astrology-by-date-of-birth', anchor: 'astrology by date of birth', after: 'explains how date, time and place work together in a chart.' },
  { title: 'Mangal Dosha from Lagna, Moon and Venus', text: 'Lagna is the ascendant, or the sign rising at birth, and it is the primary reference on this page. Mars in houses one, two, four, seven, eight or twelve from Lagna meets the displayed six-house rule. The calculator also counts Mars from Moon and Venus separately because some practitioners use those reference points when discussing relationships. These are not three independent predictions and are not a percentage score. They are three transparent chart comparisons. A Moon-based result can differ from the Lagna result without any mathematical conflict. Different websites can also differ because they use different ayanamsas, house systems, birth details or traditional rules.', link: '/rashi-calculator-by-date-of-birth', anchor: 'the Rashi calculator', after: 'helps you find the Moon sign used in the supplementary check.' },
  { title: 'Cancellation factors and compatibility', text: 'Traditional readers may review the dignity of Mars and the influence of Jupiter after a Manglik indication appears. This tool flags selected factors for review, such as Mars in its own or exaltation sign and selected Jupiter relationships. These are not presented as an automatic cancellation because traditions differ and one factor cannot replace complete chart interpretation. A Manglik label also cannot decide whether two people should marry. Relationship decisions depend on consent, communication, shared values, trust and real behaviour. Within traditional chart work, readers may consider the seventh house, Venus, Jupiter, divisional charts, planetary periods and both partners’ charts. A one-person calculator cannot make that complete comparison.', link: '/free-kundali-matching', anchor: 'free Kundli matching', after: 'can be used to review two charts as a separate compatibility exercise.' },
  { title: 'Accurate calculation and responsible interpretation', text: 'This page separates calculation from interpretation. Swiss Ephemeris and Lahiri sidereal settings calculate planetary positions, while the stated house rule determines the displayed Manglik checks. Correct coordinates, timezone handling and a clear house method improve the reproducibility of the numerical output. They do not establish astrology as a scientifically proven way to predict a marriage outcome. The result is best used as a transparent description of a traditional chart classification. Do not let a calculator pressure you into a purchase, ritual or urgent decision. If you want to discuss the context, use Vedika AI for questions about the displayed result or consult a qualified person you trust.', link: '/free-ai-astrologer-chat', anchor: 'Chat to Vedika AI', after: 'to ask follow-up questions after reviewing your calculation.' },
  { title: 'Manglik Dosha and age', text: 'A natal chart records planetary positions for a particular birth moment. Mars does not move to another natal house when someone reaches a particular age, so the calculator does not remove a Manglik indication after age 28 or any other birthday. Some traditional readers may discuss maturity, changing life circumstances or the timing of active planetary periods. Those are interpretation topics, separate from the basic natal Mars house check. Keeping them separate helps prevent an age-based claim from being mistaken for a mathematical calculation. If timing is your question, review it alongside the active Dasha rather than changing the original Mars position.', link: '/dasha-calculator/', anchor: 'the Dasha calculator', after: 'shows planetary timing separately from the natal Manglik result.' },
  { title: 'Why calculators can disagree', text: 'When two Manglik calculators return different answers, first compare the input rather than assuming one system is wrong. A recorded birth time may have been entered in AM or PM incorrectly, a timezone may have been applied from the current place instead of the birthplace, or a city with the same name may have been selected in another country. Websites may also use tropical rather than sidereal positions, a different ayanamsa, unequal houses rather than whole signs, or a different list of Mars houses. This calculator identifies its approach so you can compare the result fairly: Lahiri sidereal positions, whole-sign houses, and the six-house convention including the second house.', link: '/nakshatra-calculator', anchor: 'the Nakshatra calculator', after: 'uses the same birth-detail approach for another important Vedic chart calculation.' },
  { title: 'Mangal Dosha and marriage decisions', text: 'A calculator can calculate positions but it cannot assess a relationship. A Manglik or non-Manglik result should never be used to shame someone, reject a person, or make an urgent decision about marriage. Relationships are built through mutual consent, respect, communication, safety and shared practical choices. Traditional astrology itself contains many factors that readers may consider beyond Mars, including the seventh house, Venus, Jupiter, Navamsa and the charts of both people. This tool deliberately reports the named Mars rule rather than claiming to predict divorce, spouse harm or relationship success. If a result causes worry, speak with the person involved and seek calm, informed guidance.', link: '/ai-marriage-prediction-by-date-of-birth', anchor: 'marriage chart guidance', after: 'offers more context for traditional relationship questions.' },
  { title: 'Using the result after calculation', text: 'After you calculate, note the Lagna, Moon and Venus results separately and keep the birthplace and timezone shown with the output. These details make it easier to reproduce or discuss the result later. A result that changes after correcting a time or place is a sign that the input needs further verification. You can generate a full Kundli to inspect the wider chart, use Kundli matching if you have consent and accurate details for both people, or ask a focused question in Vedika AI. The calculator itself remains free. It is designed to explain where Mars is counted from, so you can make a considered decision about what the traditional label does and does not mean.', link: '/ai-kundli-analysis', anchor: 'AI Kundli analysis', after: 'can help explain the wider birth chart after this focused check.' },
];
